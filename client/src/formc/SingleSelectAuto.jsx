...
export default function ({ name, formValues, endpoint, errors, onChangeForm, label = null, id = null }) {
    ...
    const fetchDefaultOptions = async () => {
        try {
            const response = await axios.get(`${config.api}/st/auto/${endpoint}`, config.header())
    ...
    const fetchSelectedOption = async () => {
        try {
            ...
            const response = await axios.post(`${config.api}/st/auto/${endpoint}`, { id: selected }, config.header())
    ...
    const onSearch = async (e) => {
        ...
        const response = await axios.get(`${config.api}/st/auto/${endpoint}?search=${search}`, config.header())
        setOptions(response.data)
    }
                            ...
                            <div className="option-items">
                                {
                                    options.map(option => {
                                        ...
                                        return (
                                            <div key={option.id} className={'assign-item-list ' + checked} onClick={() => onClickItem(option.id)}>
